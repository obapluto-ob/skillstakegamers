# Add unique features routes to app.py
with open('app.py', 'r') as f:
    content = f.read()

# Add the unique dashboard route if not exists
if 'unique_dashboard' not in content:
    unique_route = '''
...
'''
    # Find position to insert (before the last route)
    pos = content.rfind('@app.route')
    if pos != -1:
        # Find the end of that route function
        next_route = content.find('@app.route', pos + 1)
        if next_route == -1:
            next_route = len(content)
        content = content[:next_route] + unique_route + '\n' + content[next_route:]

# Add other unique features routes
new_routes = '''
...
def get_live_matches():
    try:
        with get_db_connection() as conn:
            c = conn.cursor()
            c.execute("""SELECT m.id, u1.username, u2.username, m.bet_amount FROM matches m
                       JOIN users u1 ON m.player1_id = u1.id JOIN users u2 ON m.player2_id = u2.id
