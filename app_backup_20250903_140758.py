...
def analyze_screenshot(screenshot_data, claimed_result, game_type):
    ...
    if analysis_result == 'VALID_GAME_SCREENSHOT':
        ...
        if analysis_result == 'VALID_GAME_SCREENSHOT':
            # Additional strict checks for each game
            if game_type == 'pubg_mobile':
                # PUBG must have orange/red UI elements
                hsv = cv2.cvtColor(opencv_image, cv2.COLOR_BGR2HSV)
                orange_mask = cv2.inRange(hsv, (10, 100, 100), (25, 255, 255))
                red_mask = cv2.inRange(hsv, (0, 120, 70), (10, 255, 255))
                ui_pixels = cv2.countNonZero(orange_mask) + cv2.countNonZero(red_mask)
                
                if ui_pixels > (image.width * image.height * 0.05):
                    game_validation_passed = True
                    detection_reason = 'PUBG UI elements detected and validated'
                else:
                    fraud_score += 70
                    fraud_reasons.append('No PUBG UI elements detected')
            
            elif game_type == 'fifa_mobile':
                # FIFA must have green field elements
                hsv = cv2.cvtColor(opencv_image, cv2.COLOR_BGR2HSV)
                green_mask = cv2.inRange(hsv, (35, 40, 40), (85, 255, 255))
                field_pixels = cv2.countNonZero(green_mask)
                
                if field_pixels > (image.width * image.height * 0.1):
                    game_validation_passed = True
                    detection_reason = 'FIFA field elements detected and validated'
                else:
                    fraud_score += 70
                    fraud_reasons.append('No FIFA field elements detected')
            
            elif game_type == 'cod_mobile':
                # COD must have proper contrast (dark UI with bright elements)
                gray = cv2.cvtColor(opencv_image, cv2.COLOR_BGR2GRAY)
                ...
                if dark_pixels > 0 and bright_pixels > 0:
                    contrast_ratio = bright_pixels / dark_pixels
                    if 0.1 < contrast_ratio < 2.0:
                        game_validation_passed = True
                        detection_reason = 'COD UI contrast validated'
                    else:
                        fraud_score += 70
                        fraud_reasons.append('COD UI contrast invalid')
                else:
                    fraud_score += 70
                    fraud_reasons.append('No COD UI elements detected')
            else:
                fraud_score += 80
...
def send_stream_chat():
    print(f"Chat request received. Session: {session}")
    ...
    try:
        ...
        print(f"Stream ID: {stream_id}, Message: {message}")
        ...
        try:
            c.execute('ALTER TABLE stream_chat ADD COLUMN user_id INTEGER')
        except:
            pass
        ...
        c.execute('INSERT INTO stream_chat (stream_id, username, message) VALUES (?, ?, ?)',
...
def send_gift():
    ...
    # Get stream owner
    c.execute('SELECT user_id FROM streams WHERE id = ?', (stream_id,))
    stream_owner = c.fetchone()
    
    if not stream_owner:
        return jsonify({'error': 'Stream not found'}), 404
    
    stream_owner_id = stream_owner[0]
    
    # Calculate platform fee (10%)
    platform_fee = amount * 0.10
    streamer_earnings = amount * 0.90
    
    # Process gift transaction
    c.execute('UPDATE users SET balance = balance - ? WHERE id = ?', (amount, session['user_id']))
    c.execute('UPDATE users SET balance = balance + ? WHERE id = ?', (streamer_earnings, stream_owner_id))
    
    # Record transactions
    c.execute('''INSERT INTO transactions (user_id, type, amount, description)
                 VALUES (?, ?, ?, ?)''',
             (session['user_id'], 'gift_sent', -amount, f'Gift {emoji} sent to stream #{stream_id} - KSh {amount}'))
    
    c.execute('''INSERT INTO transactions (user_id, type, amount, description)
                 VALUES (?, ?, ?, ?)''',
             (stream_owner_id, 'gift_received', streamer_earnings, f'Gift {emoji} received from {session["username"]} - KSh {streamer_earnings:.0f} (after 10% fee)'))
    
    # Platform commission
    c.execute('''INSERT INTO transactions (user_id, type, amount, description)
                 VALUES (?, ?, ?, ?)''',
             (1, 'gift_commission', platform_fee, f'10% commission from gift - Stream #{stream_id}'))
    
    # Track gift for rewards
    c.execute('''CREATE TABLE IF NOT EXISTS gift_tracking (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        user_id INTEGER,
        stream_id INTEGER,
        amount REAL,
    ...
    c.execute('INSERT INTO gift_tracking (user_id, stream_id, amount, emoji) VALUES (?, ?, ?, ?)',
             (session['user_id'], stream_id, amount, emoji))
    
    # Check for gift rewards (TikTok style)
    c.execute('SELECT COALESCE(SUM(amount), 0) FROM gift_tracking WHERE user_id = ?', (session['user_id'],))
    total_gifts = c.fetchone()[0]
    
    reward_message = ''
    if total_gifts >= 10000:  # KSh 10,000 total gifts
        reward_message = '🏆 VIP Gifter Badge Unlocked!'
    elif total_gifts >= 5000:  # KSh 5,000 total gifts
        reward_message = '💎 Diamond Supporter Badge Unlocked!'
    elif total_gifts >= 1000:  # KSh 1,000 total gifts
        reward_message = '⭐ Star Supporter Badge Unlocked!'
...
def get_smart_welcome(stream_id):
    ...
    messages = {
        'pubg_mobile': f'🔥 {session["username"]} is dropping into Sanhok! Who\'s ready for some chicken dinners? 🍗 Drop your predictions!',
        'cod_mobile': f'💥 {session["username"]} locked and loaded for some COD action! Warzone or MP? Let\'s see those headshots! 🎯',
        'fifa_mobile': f'⚽ {session["username"]} is on the pitch! Who\'s your favorite team? Let\'s see some goals! 🥅',
        'efootball': f'⚽ {session["username"]} ready to dominate the field! Messi or Ronaldo? Drop your predictions! 🏆',
        'fortnite': f'🏗️ {session["username"]} building for Victory Royale! Who\'s ready for some epic builds? 🎮',
        'valorant': f'🎯 {session["username"]} locked in for ranked! Ace incoming? Drop your agent predictions! 💀'
    }
    
    default_msg = f'🎮 {session["username"]} is LIVE! Ready for some epic gaming? Drop a follow and let\'s get this party started! 🚀'
